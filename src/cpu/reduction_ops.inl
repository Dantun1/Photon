...
NDArray<T> reduction_op_kernel(const NDArray<T> &a, const DimVec &axes, Op op, T init_val, bool keepdims)
{
  ...
  if (axes.size() > src_shape.size())
    throw std::invalid_argument("Too many axes provided for reduction operation");
  ...
  for (int axis : axes)
  {
    ...
    if (axis < 0)
      throw std::invalid_argument("invalid axis provided, too negative");
...
NDArray<T> NDArray<T>::sum(const DimVec &axes, bool keepdims) const
{
  ...
  return reduction_op_kernel(*this, axes, [](T a, T b)
                             { return a + b; }, (T)0, keepdims);
}
...
NDArray<T> NDArray<T>::max(const DimVec &axes, bool keepdims) const
{
  ...
  return reduction_op_kernel(*this, axes, [](T a, T b)
                             { return std::max(a, b); }, std::numeric_limits<T>::lowest(), keepdims);
}
template <typename T>
NDArray<T> NDArray<T>::min(const DimVec &axes, bool keepdims) const
{
  ...
  return reduction_op_kernel(*this, axes, [](T a, T b)
                             { return std::min(a, b); }, std::numeric_limits<T>::max(), keepdims);
}
